...
void init_vertices_downtoup(GFX3D_Vertex *tmp_v, int  m_CurFrame,
        TDRECT *rect, GodLightPosition position)
{
	int x, y;
	float curLeft;
	float coef;
...
void update_vertices_downtoup(GFX3D_Vertex *v, GFX3D_Vertex *tmp_v,
        int Cur_line, TDRECT *rect, GodLightPosition position)
{
	int x, y;
	float width = (float)(rect->right - rect->left);
	float height = (float)(rect->bottom - rect->top);
	...
	for(y = 0; y <= position.DEFAULT_Y_SUBDIV; y++)
	{
		...
		// {
			...
			{
                float x0 = Lerp( 0.0, Lx, (float)1.0/((position.DEFAULT_Y_SUBDIV-y+1)/4.0) );
                printf("NALL 7&&&&&& %f\n", x0);
                v->fX = x0; 
				v->fY = By + y * (Ty - By) / position.DEFAULT_Y_SUBDIV;
				v->fZ = 0.0;
				// v->fS = x * x_mul;
				// v->fT = y * y_mul;
				v++;
...
void GodLight_DOWNTOUP()
{
    ...
    for(i = 0; i <= 1+godlightposition.DEFAULT_Y_SUBDIV; i++)
...
int main(int argc, char** argv)
{
    ...
    glutDisplayFunc(GodLight_DOWNTOUP);
